...
SctCopyright (c) 2004 - 2010, Intel Corporation. All rights reserved.<BR>
...
--*/
{
  ...
  CpuHob.Raw  = HobStart;
  CpuHob.Raw  = SctGetHob (EFI_HOB_TYPE_CPU, CpuHob.Raw);
...
--*/
{
  ...
  FirmwareVolumeHob.Raw = SctGetHob (EFI_HOB_TYPE_FV, *HobStart);
...
--*/
{
  ...
  FirmwareVolumeHob.Raw = SctGetHob (EFI_HOB_TYPE_FV2, *HobStart);
...
--*/
{
  ...
  for (Status = EFI_NOT_FOUND; EFI_ERROR (Status);) {
    ...
    GuidHob.Raw = SctGetHob (EFI_HOB_TYPE_GUID_EXTENSION, *HobStart);
    if (GuidHob.Header->HobType == EFI_HOB_TYPE_GUID_EXTENSION) {
      if (SctCompareGuid (Guid, &GuidHob.Guid->Name) == 0) {
        Status  = EFI_SUCCESS;
...
EFI_STATUS
SctGetIoPortSpaceAddressHobInfo (
  IN  VOID                  *HobStart,
