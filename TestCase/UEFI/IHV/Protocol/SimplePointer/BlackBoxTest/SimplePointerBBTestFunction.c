  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    StandardLib->RecordAssertion (
                   ...
                   EFI_TEST_ASSERTION_FAILED,
                   gTestGenericFailureGuid,
                   L"BS.LocateHandleBuffer - Locate Simple Pointer handles",
                   L"%a:%d:Status - %r",
                   __FILE__,
                   (UINTN)__LINE__,
                   Status
  ...
  //
  // Scan all simple pointer handles
  //
  for (Index = 0; Index < NoHandles; Index++) {
    ...
    if (SimplePointer != ClientInterface) {
      continue;
    }
    ...
    StandardLib->RecordAssertion (
                   StandardLib,
                   AssertionType,
                   gSimplePointerInterfaceTestAssertionGuid080,
                   L"EFI_SIMPLE_POINTER_PROTOCOL - The handle of a Simple Pointer Protocol must also have a Device Path Protocol",
                   L"%a:%d: Status = %r, Expected = %r",
                   __FILE__,
                   (UINTN)__LINE__,
                   Status,
